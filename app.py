...
# FUNCIÓN INDEPENDIENTE PARA COMPRIMIR Y PROCESAR CON LA IA
def analizar_un_archivo_con_ia(archivo):
    try:
        ...
        # --- CAMINO RÁPIDO Y FORMATOS DIGITALES ---
        if ext == "xml":
            archivo.seek(0)
        ...
        # --- CAMINO DE IMÁGENES O PDF (CON COMPRESIÓN AUTOMÁTICA) ---
        model = genai.GenerativeModel('gemini-1.5-flash')
        prompt = "Analiza este comprobante de pago de Perú y extrae exclusivamente los siguientes campos en un formato JSON limpio y válido, sin bloques de código ```json: {\"ruc_emisor\": \"RUC de 11 dígitos\", \"razon_social\": \"Nombre proveedor\", \"fecha_emision\": \"AAAA-MM-DD\", \"serie\": \"Serie\", \"numero\": \"Número correlativo\", \"total\": 0.00, \"categoria_gasto\": \"Categoría contable corta de 2 a 4 palabras\"}"
        
        if ext in ["jpg", "jpeg", "png"]:
            # TRUCO: Compresión automática en memoria con la librería Pillow
            img = Image.open(io.BytesIO(bytes_archivo))
            img = img.convert("RGB")
            out_img = io.BytesIO()
            img.save(out_img, format="JPEG", quality=40) # Reduce el peso un 80% manteniendo la legibilidad
            documento_data = {'mime_type': 'image/jpeg', 'data': out_img.getvalue()}
        else:
            # Si es PDF, se envía optimizado directo a la visión de Gemini
...
# FUNCIÓN AUXILIAR PARA GENERAR LA MATEMÁTICA FISCAL PERUANA
...
# Caja de subida masiva avanzada
...
if archivos_subidos:
    ...
    # EL SECRETO DE LA VELOCIDAD: Procesamiento en hilos en paralelo (ThreadPoolExecutor)
    # Envía todos los archivos al mismo tiempo al servidor en vez de ir uno por uno
